...
**Status:** Interface, planner, Firefly dry-run executor, live Firefly approval gate, and signed result publisher implemented; chain execution remains gated.
...
7. A contract executor mints/transfers/records on chain and emits the same recorded/failed result events.
...
- Matching signed operator approval for `firefly_live_execution`.
